...
function register(context, languageClient) {
    context.subscriptions.push(vscode.commands.registerTextEditorCommand("imandra.caseSplit", async (editor) => {
        ...
        try {
            const response = await languageClient.sendRequest(imandra_language_server_1.remote.server.giveCaseAnalysis, params);
            if (null != response)
