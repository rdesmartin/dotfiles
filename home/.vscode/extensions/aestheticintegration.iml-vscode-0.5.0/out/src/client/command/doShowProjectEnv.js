...
function register(context, languageClient) {
    context.subscriptions.push(vscode.commands.registerTextEditorCommand("imandra.showProjectEnv", async (editor) => {
        const docURI = {
            uri: editor.document.uri.toString(),
        };
        const projectEnv = await languageClient.sendRequest(imandra_language_server_1.remote.server.giveProjectEnv, docURI);
