...
class Session {
    ...
    makeOpamCmd(cmd) {
        if (this.hasOpam) {
            if (this.hasOpam === "1") {
                return [
                    this.settings.imandra.path.opam,
                    ["env", "exec", "--switch", this.settings.imandra.path.switch, "--", cmd],
                ];
            }
            else {
                return [this.settings.imandra.path.opam, ["exec", "--switch", this.settings.imandra.path.switch, "--", cmd]];
            }
        }
        else {
            return [cmd, []];
    ...
    onDidChangeConfiguration({ settings }) {
        return __awaiter(this, void 0, void 0, function* () {
            const oldImandraMerlinPath = this.settings.imandra.path.imandra_merlin;
            this.settings = Object.assign({}, this.settings, settings);
            if (this.settings.imandra.path.imandra_merlin !== oldImandraMerlinPath) {
