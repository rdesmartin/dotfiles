...
function default_1(session) {
    return support.cancellableHandler(session, ({ textDocument }, token) => __awaiter(this, void 0, void 0, function* () {
        ...
        const languages = new Set(session.settings.imandra.server.languages);
        if (languages.size < 1)
            return [];
        const allowedFileKinds = [];
        if (languages.has("imandra"))
            allowedFileKinds.push("iml");
        if (languages.has("imandra-reason"))
            allowedFileKinds.push("ire");
