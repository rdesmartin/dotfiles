    ...
    readNative(skipEncoding) {
        switch (this.source) {
            ...
            case host_1.Host.WSL:
                ...
                const localappdata = vscode_uri_1.URI.file(localappdataFile).toString(skipEncoding);
                let match = null;
                if (null != (match = uri.match(/^file:\/\/\/mnt\/([a-zA-Z])\/(.*)$/))) {
                    match.shift();
                    const drive = match.shift();
                    const rest = match.shift();
                    return vscode_uri_1.URI.parse(`file:///${drive}:/${rest}`);
                }
                if (null != (match = uri.match(/^file:\/\/\/home\/(.+)$/))) {
                    match.shift();
                    const rest = match.shift();
                    return vscode_uri_1.URI.parse(`${localappdata}/lxss/home/${rest}`);
                }
        ...
        switch (this.source) {
            case host_1.Host.Native:
                ...
                if (null != (match = uri.match(/^file:\/\/\/([a-zA-Z]):(.*)$/))) {
                    ...
                    const drive = match.shift();
                    const rest = match.shift();
                    return vscode_uri_1.URI.parse(`file:///mnt/${drive}${rest}`);
                }
                throw new Error("unreachable");
            case host_1.Host.WSL:
                return this.uri;
